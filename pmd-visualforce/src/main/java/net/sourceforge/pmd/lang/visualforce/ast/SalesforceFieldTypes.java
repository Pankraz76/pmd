 ...
 * Visualforce to the type of the field. The fields are identified by in a case
 * insensitive manner.
 */
abstract class SalesforceFieldTypes {
    /**
     * Cache of lowercase variable names to the variable type declared in the
     * field's metadata file.
     */
    private final Map<String, DataType> variableNameToVariableType;

    /**
     * Keep track of which variables were already processed. Avoid processing if a
     * page repeatedly asks for an entry which we haven't previously found.
     */
     ...
     *
     * @param expression
     *            expression literal as declared in the Visualforce page
     * @param vfFileName
     *            file name of the Visualforce page that contains expression. Used
     *            to resolve relative paths included in {@code metadataDirectories}
     * @param metadataDirectories
     *            absolute or relative list of directories that may contain the
     *            metadata corresponding to {@code expression}
    ...
    /**
     * Stores {@link DataType} in a map using lower cased {@code expression} as the
     * key.
     * 
     * @param expression
     *            expression literal as declared in the Visualforce page
     * @param dataType
     *            identifier determined for
    ...
    /**
     * @return true if the expression has previously been stored via
    ...
    /**
     * Subclasses should attempt to find the {@code DataType} of {@code expression}
     * within {@code metadataDirectories}. The subclass should store the value by
     * invoking {@link #putDataType(String, DataType)}.
     *
     * @param expression
     *            expression as defined in the Visualforce page, case is preserved
     * @param metadataDirectories
     *            list of directories that may contain the metadata corresponding to
     *            {@code expression}
     */
