 ...
 * {@link net.sourceforge.pmd.lang.visualforce.ast.ASTIdentifier} children that
 * represent an IdentifierDotted construct. An IdentifierDotted is of the form
 * {@code MyObject__c.MyField__c}.
 */
class VfExpressionTypeVisitor extends VfVisitorBase<Void, Void> {
    ...
    /**
     * List of all Apex Class names that the VF page might refer to. These values
     * come from either the {@code controller} or {@code extensions} attribute.
     */
    ...
    /**
     * Gather names of Controller, Extensions, and StandardController. Each of these
     * may contain the identifier referenced from the Visualforce page.
     */
    @Override
    ...
    private static String getAttrValue(ASTAttribute attr) {
        return attr.firstChild(ASTAttributeValue.class).firstChild(ASTText.class).getImage();
    }

    /**
     * Invoke {@link ASTExpression#getDataNodes()} on all children of {@code node}
     * and attempt to determine the {@link DataType} by looking at Apex or
    ...
    public Void visit(ASTElExpression node, Void data) {
        for (Map.Entry<VfTypedNode, String> entry : getDataNodeNames(node).entrySet()) {
            ...
            // The example below will display "Name From Inner Class" instead of the Account
            // name
            ...
            // Try to find the identifier in a CustomField if it wasn't found in an Apex
            // class and the identifier corresponds
            // to the StandardController.
            if (type == null) {
                if (parts.length >= 2 && standardControllerName != null
    ...
    /**
     * Invoke {@link ASTExpression#getDataNodes()} for all {@link ASTExpression}
     * children of {@code node} and return the consolidated results.
     */
