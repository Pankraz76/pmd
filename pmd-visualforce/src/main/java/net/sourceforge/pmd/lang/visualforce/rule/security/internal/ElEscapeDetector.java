...
/**
 * Helps detect visualforce encoding in EL Expressions (porting over code
 * previously living in VfUnescapeElRule for reusability)
 */
...
public final class ElEscapeDetector {
    ...
    private static final Set<String> SAFE_BUILTIN_FUNCTIONS = new HashSet<>(Arrays.asList(
            // These DateTime functions accept or return dates, and therefore don't need
            // escaping.
            "addmonths", "date", "datevalue", "datetimevalue", "day", "hour", "millisecond", "minute", "month", "now",
            "second", "timenow", "timevalue", "today", "weekday", "year",
            // These Logical functions accept or return booleans, and therefore don't need
            // escaping.
            ...
            // These Text functions are safe, either because of what they accept or what
            // they return.
            "begins", "br", "casesafeid", "contains", "find", "getsessionid", "ispickval", "len",
            // These Advanced functions are safe because of what they accept or what they
            // return.
    ...
    /**
     * Given an ASTExpression node, determines whether that expression and any
     * expressions under it are properly escaped.
     * 
     * @param expression
     *            - Represents a VF expression
     * @param escapes
     *            - The escape operations that are acceptable in this context
    ...
    public static boolean expressionRecursivelyValid(final ASTExpression expression, final Set<Escaping> escapes) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (child instanceof ASTIdentifier) {
                ...
                if (nextNode instanceof ASTArguments || nextNode instanceof ASTDotExpression) {
                    // If the next node is Arguments or Dot expression, that means this Identifier
                    // represents the name
                    // of a function, or some kind of object. In that case, we might be okay. So
                    // we'll store the name
                    // and keep going.
                    prevId = child.getImage();
                    continue;
                }
                // If there's no next node, or the next node isn't one of the desired types,
                // then this Identifier is a raw
                // variable.
                if (typedNodeIsSafe((ASTIdentifier) child)) {
                    // If the raw variable is of an inherently safe type, we can keep going.
                    continue;
                }
                // If the raw variable isn't inherently safe, then it's dangerous, and must be
                // escaped.
                return false;
            } else if (child instanceof ASTArguments) {
                // An Arguments node means we're looking at some kind of function call.
                // If it's one of our designated escape functions, we're in the clear and we can
                // keep going.
                // Also, some built-in functions are inherently safe, which would mean we're
                // good to continue.
                if (functionIsEscape(prevId, escapes) || functionInherentlySafe(prevId)) {
                    continue;
                }

                // Otherwise, identify the argument expressions that must be checked, and add
                // them to the list.
                ...
                // If the variable is one of the definitely-safe global variables, then we're in
                // the clear.
                if (isSafeGlobal(prevId)) {
                    continue;
                }
                // If the node after this one is also a Dot expression, then this is a chained
                // access, and we can't make
                // any final judgements.
                if (getNextSibling(child) instanceof ASTDotExpression) {
                    continue;
                }
                // If none of those things are true, then we need to determine whether the field
                // being accessed is
                // definitely safe.
                ASTIdentifier propId = child.firstChild(ASTIdentifier.class);
                // If there's an identifier for a field/property, we need to check whether that
                // property is inherently safe,
                // either because it corresponds to a safe field or because its data type is
                // known to be safe.
        ...
        // Just because there's nothing immediately wrong with this node doesn't mean
        // its children are guaranteed to be
        // fine. Iterate over all of the children and make a recursive call. If any of
        // those calls return false, we need
    ...
    /**
     * Indicates whether the provided function name corresponds to any of the
     * provided escape functions.
     * 
     * @param functionName
     *            - The name of a VF function
     * @param escapes
     *            - A set of acceptable escape functions (e.g., JSENCODE,
     *            HTMLENCODE, etc)
    ...
    private static boolean functionIsEscape(String functionName, Set<Escaping> escapes) {
        // If one of the escapes we were passed is ANY, then we should replace the
        // provided set with one that contains
    ...
    /**
     * Certain built-in functions are inherently safe and don't require any
     * escaping. This method determines whether the function name provided
     * corresponds to one of those methods.
     * 
     * @param functionName
     *            - The name of a VF function
    ...
    /**
     * Given a function name and a node containing its arguments, returns the
     * ASTExpression nodes corresponding to arguments that require escaping.
     * Frequently, this will be all arguments, but not always.
     * 
     * @param functionName
     *            - The name of a function being called
     * @param arguments
     *            - Contains the ASTExpression nodes representing the function's
     *            arguments
     * @return - ASTExpression list containing all arguments that are vulnerable to
     *         XSS.
     */
    private static List<ASTExpression> getXssableArguments(String functionName, ASTArguments arguments) {
        ...
        if (argCount != 0) {
            ...
            // See if the function name corresponds to one of the built-in functions that
            // don't require us to examine
            // every argument.
            if ("case".equals(lowerCaseName)) {
                // CASE accepts (exx, val1, res1, val2, res2, ...else_res). We want resX and
                // else_res.
                ...
                // If this is a function that has at least 3 args, and the third arg is XSSable,
                // add 2 to the array.
                if (argCount > 2 && FUNCTIONS_WITH_XSSABLE_ARG2.contains(lowerCaseName)) {
                    checkAllArgs = false;
                    indicesToCheck.add(2);
                }
                // If the function has no known pattern for argument checking, all arguments
                // must be checked.
                if (checkAllArgs) {
            ...
            // Add each of the targeted arguments to the return array if they represent an
            // Expression node. (They always
    ...
    /**
     * VF has global variables prefixed with a '$'. Some of those are inherently
     * safe to access, and this method determines whether the provided ID
     * corresponds to one of those globals.
     * 
     * @param id
    ...
    /**
     * Determines whether the property being referenced is inherently safe, or if it
     * requires XSS escaping.
     * 
     * @param propertyName
     *            - The name of a field or property being referenced.
    ...
    /**
     * Return true if the type of all data nodes can be determined and none of them
     * require escaping
     * 
     * @param expression
     */
