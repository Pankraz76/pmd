...
//@formatter:off
/**
 * Provides factory methods for common property types. Note: from 7.0.0 on, this
 * will be the only way to build property descriptors. Concrete property classes
 * and their constructors/ builders will be gradually deprecated before 7.0.0.
 *
 * <h2>Usage</h2>
 *
 * Properties are a way to make your rule configurable by letting a user fill in
 * some config value in their ruleset XML.
 *
 * As a rule developer, to declare a property on your rule, you must:
 * <ul>
 * <li>Build a {@link PropertyDescriptor} using one of the factory methods of
 * this class, and providing the {@linkplain PropertyBuilder builder} with the
 * required info.
 * <li>Define the property on your rule using
 * {@link PropertySource#definePropertyDescriptor(PropertyDescriptor)}. This
 * should be done in your rule's constructor.
 * </ul>
 *
 * You can then retrieve the value configured by the user in your rule using
 * {@link PropertySource#getProperty(PropertyDescriptor)}.
 *
 * <h2>Example</h2>
 *
 * <pre>
...
public final class PropertyFactory {
    ...
    /**
     * Default delimiter for all properties. Note that in PMD 6 this was the pipe
     * character {@code |}, while now it is {@value}. In PMD 6, numeric properties
     * had a different delimiter, whereas in PMD 7, all properties have the same
     * delimiter.
     */
    ...
    /**
     * Returns a builder for an integer property. The property descriptor will by
     * default accept any value conforming to the format specified by
     * {@link Integer#parseInt(String)}, e.g. {@code 1234} or {@code -123}.
     *
     * <p>
     * Note that that parser only supports decimal representations.
     *
     * <p>
     * Acceptable values may be further refined by
     * {@linkplain PropertyBuilder#require(PropertyConstraint) adding constraints}.
     * The class {@link NumericConstraints} provides some useful ready-made
     ...
     * format of the individual items is the same as for
     * {@linkplain #intProperty(String) intProperty}.
     *
     * @param name
     *            Name of the property to build
     *
    ...
    /**
     * Returns a builder for a long integer property. The property descriptor will
     * by default accept any value conforming to the format specified by
     * {@link Long#parseLong(String)}, e.g. {@code 1234455678854}.
     *
     * <p>
     * Note that that parser only supports decimal representations, and that neither
     * the character L nor l is permitted to appear at the end of the string as a
     * type indicator, as would be permitted in Java source.
     *
     * <p>
     * Acceptable values may be further refined by
     * {@linkplain PropertyBuilder#require(PropertyConstraint) adding constraints}.
     ...
     * format of the individual items is the same as for
     * {@linkplain #longIntProperty(String)} longIntProperty}.
     *
     * @param name
     *            Name of the property to build
     *
    ...
    /**
     * Returns a builder for a double property. The property descriptor will by
     * default accept any value conforming to the format specified by
     * {@link Double#valueOf(String)}, e.g. {@code 0}, {@code .93}, or {@code 1e-1}.
     * Acceptable values may be further refined by
     * {@linkplain PropertyBuilder#require(PropertyConstraint) adding constraints}.
     * The class {@link NumericConstraints} provides some useful ready-made
     * constraints for that purpose.
     ...
     * The format of the individual items is the same as for
     * {@linkplain #doubleProperty(String) doubleProperty}.
     *
     * @param name
     *            Name of the property to build
     *
    ...
    /**
     * Returns a builder for a regex property. The value type of such a property is
     * {@link java.util.regex.Pattern}. For this use case, this type of property
     * should be preferred over {@linkplain #stringProperty(String) stringProperty}
    ...
    /**
     * Returns a builder for a string property. The property descriptor will accept
     * any string, and performs no expansion of escape sequences (e.g. {@code \n} in
     * the XML will be represented as the character sequence '\' 'n' and not the
     * line-feed character '\n'). The value of a string attribute is trimmed. This
     ...
     * format of the individual items is the same as for
     * {@linkplain #stringProperty(String) stringProperty}.
     *
     * @param name
     *            Name of the property to build
     *
    ...
    /**
     * Returns a builder for a character property. The property descriptor will
     * accept any single character string. No unescaping is performed other than
     * what the XML parser does itself. That means that Java escape sequences are
     * not expanded: e.g. "\n", will be represented as the character sequence '\'
     * 'n', so it's not a valid value for this type of property. On the other hand,
     * XML character references are expanded, like {@literal &amp;} ('&amp;') or
     * {@literal &lt;} ('&lt;').
     *
     * @param name
     *            Name of the property to build
     ...
     * format of the individual items is the same as for
     * {@linkplain #charProperty(String) charProperty}.
     *
     * @param name
     *            Name of the property to build
     *
    ...
    /**
     * Returns a builder for a boolean property. The boolean is parsed from the XML
     * using {@link Boolean#valueOf(String)}, i.e. the only truthy value is the
     * string "true", and all other string values are falsy.
     *
     * @param name
     *            Name of the property to build
     *
    ...
    /**
     * Returns a builder for an enumerated property. Such a property can be defined
     * for any type {@code <T>}, provided the possible values can be indexed by
     * strings. This is enforced by passing a {@code Map<String, T>} at construction
     * time, which maps labels to values. If {@link Map#get(Object)} returns null
     * for a given label, then the value is rejected. Null values are hence
     * prohibited.
     *
     * @param name
     *            Name of the property to build
     * @param nameToValue
     *            Map of labels to values. The null key is ignored.
     * @param <T>
    ...
    public static <T> GenericPropertyBuilder<T> enumProperty(String name, Map<String, T> nameToValue) {
        PropertySerializer<T> parser = enumerationParser(nameToValue,
                t -> Objects.requireNonNull(CollectionUtil.getKeyOfValue(nameToValue, t)));
        return new GenericPropertyBuilder<>(name, parser);
    }

    /**
     * Returns a builder for an enumerated property for the given enum class, using
     * the {@link Object#toString() toString} of its enum constants as labels.
     *
     * @param name
     *            Property name
     * @param enumClass
    ...
    /**
     * Returns a builder for an enumerated property for the given enum class, using
     * the given function to label its constants.
     *
     * @param name
     *            Property name
     * @param enumClass
     *            Enum class
     * @param labelMaker
     *            Function that labels each constant
     * @param <T>
     *            Type of the enum class
     *
     * @return A new builder
     ...
     * @throws IllegalArgumentException
     *             If the label maker returns null on some constant
     ...
     * format of the individual items is the same as for
     * {@linkplain #enumProperty(String, Map)}.
     *
     * @param name
     *            Name of the property to build
     * @param nameToValue
     ...
     * @return A new builder
     */
    public static <T> GenericCollectionPropertyBuilder<T, List<T>> enumListProperty(String name,
     ...
     *            Class of the values
     * @param labelMaker
     *            Function that associates enum constants to their label
     * @param <T>
     *            Value type of the property
     *
     * @return A new builder
     */
    public static <T extends Enum<T>> GenericCollectionPropertyBuilder<T, List<T>> enumListProperty(String name,
            Class<T> enumClass, Function<? super T, String> labelMaker) {
