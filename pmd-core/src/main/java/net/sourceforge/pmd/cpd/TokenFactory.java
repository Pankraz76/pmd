...
public interface TokenFactory extends AutoCloseable {
    ...
    /**
     * Record a token given its coordinates. Coordinates must match the requirements
     * of {@link FileLocation}, ie, be 1-based and ordered properly.
     *
     * @param image
     *            Image of the token. This will be taken into account to determine
    ...
    default void recordToken(@NonNull String image, @NonNull FileLocation location) {
        recordToken(image, location.getStartLine(), location.getStartColumn(), location.getEndLine(),
