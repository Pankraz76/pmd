...
/**
 * Base class for node produced by JJTree. JJTree specific functionality present
 * on the API of {@link Node} and {@link AbstractNode} has been moved here for
 * 7.0.0 (e.g. jjtClose methods).
 */
public abstract class AbstractJjtreeNode<B extends AbstractJjtreeNode<B, N>, N extends JjtreeNode<N>>
    ...
    /**
     * The id is an index in the constant names array generated by jjtree, it must
     * be set to some value that depends on the node type, not some arbitrary "1" or
     * "2", and not necessarily a unique value.
     */
    ...
    /**
     * Ensures that the first (resp. last) token of this node is before (resp.
     * after) the first (resp. last) token of the child at the given index. The
     * index
     */
