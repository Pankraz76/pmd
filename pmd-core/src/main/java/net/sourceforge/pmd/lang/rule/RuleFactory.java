...
class RuleFactory {
    ...
    /**
     * Decorates a referenced rule with the metadata that are overridden in the
     * given rule element.
     *
     * <p>
     * Declaring a property in the overriding element throws an exception (the
     * property must exist in the referenced rule).
     *
     * @param referencedRule
     *            Referenced rule
     * @param ruleSetReference
    ...
    public RuleReference decorateRule(Rule referencedRule, RuleSetReference ruleSetReference, Element ruleElement,
        ...
        for (Element node : DomUtils.children(ruleElement)) {
            ...
            if (DESCRIPTION.matchesElt(node)) {
                ...
                err.at(node).error(XmlErrorMessages.ERR__UNEXPECTED_ELEMENT_IN, node.getTagName(),
     ...
     *
     * <p>
     * Notes: The ruleset name is not set here. Exceptions raised from this method
    ...
    public Rule buildRule(Element ruleElement, PmdXmlReporter err) {
        ...
        try {
            ...
            throw err.at(node).error(e, "Reflection error while instantiating rule ({0})",
        ...
        for (Element node : DomUtils.children(ruleElement)) {
            if (DESCRIPTION.matchesElt(node)) {
                ...
                throw err.at(node).error(XmlErrorMessages.ERR__UNEXPECTED_ELEMENT_IN,
    ...
    private void checkVersionsAreOrdered(Element ruleElement, PmdXmlReporter err, Rule rule) {
        if (rule.getMinimumLanguageVersion() != null && rule.getMaximumLanguageVersion() != null
            ...
            throw err.at(MINIMUM_LANGUAGE_VERSION.getAttributeNode(ruleElement)).error(
                    XmlErrorMessages.ERR__INVALID_VERSION_RANGE, rule.getMinimumLanguageVersion(),
    ...
    private LanguageVersion getLanguageVersion(Element ruleElement, PmdXmlReporter err, Language language,
            SchemaConstant attrName) {
        if (attrName.hasAttribute(ruleElement)) {
            ...
            if (version == null) {
                String supportedVersions = language.getVersions().stream().map(LanguageVersion::getVersion)
                        .filter(it -> !it.isEmpty()).map(StringUtil::inSingleQuotes).collect(Collectors.joining(", "));
                String message = supportedVersions.isEmpty() ? ERR__INVALID_LANG_VERSION_NO_NAMED_VERSION
                        : ERR__INVALID_LANG_VERSION;
                throw err.at(attrName.getAttributeNode(ruleElement)).error(message, attrValue, language.getId(),
                        supportedVersions);
            }
            return version;
        }
    ...
    /**
     * Parses the properties node and adds property definitions to the builder.
     * Doesn't care for value overriding, that will be handled after the rule
     * instantiation.
     *
     * @param rule
     *            Rule builder
     ...
     *            The rule
     * @param propertiesElt
     *            The {@literal <properties>} element
     */
    ...
    private <T> void setRulePropertyCapture(Rule rule, PropertyDescriptor<T> descriptor, Element propertyElt,
            PmdXmlReporter err) {
     ...
     *            Property element
     *
     * @return True if this element defines a new property, false if this is just
     *         stating a value
     */
    ...
    /**
     * Parses a property definition node and returns the defined property
     * descriptor.
     *
     * @param propertyElement
     *            Property node to parse
     * @param err
     *            Error reporter
     *
    ...
    private static <T> PropertyDescriptor<T> propertyDefCapture(Element propertyElement, PmdXmlReporter err,
    ...
    private static <T> void parseConstraints(Element propertyElement, BuilderAndMapper<T> factory,
            PropertyBuilder<?, T> builder, PmdXmlReporter err) {
        ...
        if (min.isPresent() && max.isPresent()) {
            ...
            PropertyConstraint<T> constraint = NumericConstraints.inRange((Comparable) min.get(),
                    (Comparable) max.get());
    ...
    private static <T> Optional<Comparable<T>> parseIntoComparable(Element propertyElement, BuilderAndMapper<T> factory,
            PmdXmlReporter err, SchemaConstant schemaConstant) {
        return schemaConstant.getAttributeOpt(propertyElement)
