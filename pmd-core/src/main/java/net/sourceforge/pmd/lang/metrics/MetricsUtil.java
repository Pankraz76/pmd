...
public final class MetricsUtil {
     ...
     * @param ops
     *            List of nodes for which to compute the metric
     *
     ...
     */
    public static <O extends Node> DoubleSummaryStatistics computeStatistics(Metric<? super O, ?> key,
            Iterable<? extends O> ops, MetricOptions options) {
        ...
        return StreamSupport.stream(ops.spliterator(), false).filter(key::supports)
     ...
     *
     * @param key
     *            The key identifying the metric to be computed
     * @param node
     *            The node on which to compute the metric
     *
     * @return The value of the metric, or {@code Double.NaN} if the value couldn't
     *         be computed
     *
    ...
    /**
     * Computes a metric identified by its code on a node, possibly selecting a
     * variant with the {@code options} parameter.
     *
     * <p>
     * Note that contrary to the previous behaviour, this method throws an exception
     * if the metric does not support the node.
     *
     * @param key
     *            The key identifying the metric to be computed
     * @param node
     ...
     *            Force recomputation of the result
     *
     * @return The value of the metric
     *
     * @throws IllegalArgumentException
     ...
     */
    public static <N extends Node, R extends Number> R computeMetric(Metric<? super N, R> key, N node,
            MetricOptions options, boolean forceRecompute) {
        Objects.requireNonNull(key, NULL_KEY_MESSAGE);
        Objects.requireNonNull(options, NULL_OPTIONS_MESSAGE);
