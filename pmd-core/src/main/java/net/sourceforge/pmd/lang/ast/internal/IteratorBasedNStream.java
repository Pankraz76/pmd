...
abstract class IteratorBasedNStream<T extends Node> implements NodeStream<T> {
    ...
    @Override public <R extends Node> NodeStream<R> filterIs(Class<? extends R> rClass) {
    ...
    @NonNull protected <R extends Node> DescendantNodeStream<R> flatMapDescendants(Function<T, DescendantNodeStream<? extends R>> mapper) {
    ...
    @Override public NodeStream<T> dropLast(int n) {
        AssertionUtil.requireNonNegative("n", n);
    ...
    @Override public final <R, A> R collect(Collector<? super T, A, R> collector) {
    ...
    @Override public NodeStream<T> peek(Consumer<? super T> action) {
        return mapIter(iter -> IteratorUtil.peek(iter, action));
    }

    @Override public NodeStream<T> append(NodeStream<? extends T> right) {
        return mapIter(iter -> IteratorUtil.concat(iter, right.iterator()));
    }

    @Override public NodeStream<T> prepend(NodeStream<? extends T> right) {
    ...
    @Override public @Nullable T last() {
    ...
    private static class DescendantMapping<T extends Node, S extends Node> extends IteratorBasedNStream<S> implements DescendantNodeStream<S> {
        ...
        @Override public DescendantNodeStream<S> crossFindBoundaries(boolean cross) {
