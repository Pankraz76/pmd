...
public class RuleApplicator {
            ...
            try (TimedOperation rcto = TimeTracker.startOperation(TimedOperationCategory.RULE, rule.getName())) {
                ...
                while (targets.hasNext()) {
                    ...
                    try {
                        ...
                        reportOrRethrow(listener, rule, node, AssertionUtil.contexted(e),
                                SystemProps.isErrorRecoveryMode());
    ...
    private <E extends Throwable> void reportOrRethrow(FileAnalysisListener listener, Rule rule, Node node, E e,
            boolean reportAndDontThrow) throws E {
    ...
    private void reportException(FileAnalysisListener listener, Rule rule, Node node, Throwable e) {
        ...
        LOG.warn("Exception applying rule {} on file {}, continuing with next rule", rule.getName(),
                node.getTextDocument().getFileId().getAbsolutePath(), e);
