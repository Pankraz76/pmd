 ...
 * parameter. This enforces that eg all children of such a node are from the
 * same hierarchy (eg Java nodes only have Java nodes as parent, or as
 * children).
 *
 * <p>
 * Although subinterfaces like JavaNode profit from the added type information,
 * the Node interface and its usages in language-independent code would suffer
 * from adding a type parameter directly to {@link Node}.
 *
 * <p>
 * Type safety of the unchecked casts here is the responsibility of the
 * implementation, it should check that methods like setParent or addChild add
 * an instance of {@code <N>}.
 *
 * @param <N>
