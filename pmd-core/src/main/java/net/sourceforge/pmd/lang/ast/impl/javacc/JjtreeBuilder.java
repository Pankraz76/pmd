...
public final class JjtreeBuilder<N extends AbstractJjtreeNode<N, ?>> {
    ...
    /***
     * If non-zero, then the top "n" nodes of the stack will be injected as the
     * first children of the next node to be opened. This is not very flexible, but
     * it's enough. The grammar needs to take care of the order in which nodes are
     * opened in a few places, in most cases this just means using eg A() B() #N(2)
     * instead of (A() B()) #N, so as not to open N before A.
     */
    ...
    /**
     * Close the node scope and adds the given number of children to the node. A
     * definite node is constructed from a specified number of children. That number
     * of nodes are popped from the stack and made the children of the definite
     * node. Then the definite node is pushed on to the stack.
     */
    ...
    /**
     * Close the node scope if the condition is true. All the nodes that have been
     * pushed since the node was opened are made children of the conditional node,
     * which is then pushed on to the stack. If the condition is false the node is
     * not constructed and they are left on the stack.
     *
     * @param n
     *            Node to close
     * @param condition
     *            Whether to close the node or not
     * @param lastToken
