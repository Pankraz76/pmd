...
abstract class AxisStream<T extends Node> extends IteratorBasedNStream<T> {
    ...
    @Override @SuppressWarnings("unchecked") public final Iterator<T> iterator() {
    ...
    @Override public NodeStream<T> filter(Predicate<? super @NonNull T> predicate) {
        return copyWithFilter(filter.thenFilter(predicate));
    }

    @Override public <S extends Node> NodeStream<S> filterIs(Class<? extends S> r1Class) {
    ...
    @Override public @Nullable T first() {
        return firstImpl(filter);
    }

    @Override public <R extends Node> @Nullable R first(Class<? extends R> r1Class) {
        return firstImpl(filter.thenCast(r1Class));
    }

    @Override public @Nullable T first(Predicate<? super T> predicate) {
        return firstImpl(filter.thenFilter(predicate));
    }

    @Nullable @Override public T last() {
    ...
    @Override public <R extends Node> @Nullable R last(Class<? extends R> rClass) {
    ...
    static class FilteredAncestorOrSelfStream<T extends Node> extends AxisStream<T> {
        ...
        @Override protected <S extends Node> NodeStream<S> copyWithFilter(Filtermap<Node, ? extends S> filterMap) {
        ...
        @Override protected <O extends Node> @Nullable O firstImpl(Filtermap<? super Node, ? extends O> filter) {
    ...
    static class AncestorOrSelfStream extends FilteredAncestorOrSelfStream<Node> {
        ...
        @Nullable @Override public Node first() {
            return node;
        }
        ...
        @Override public @Nullable Node last() {
            Node last = node;
    ...
    static class FilteredDescendantStream<T extends Node> extends DescendantStreamBase<T> {
        ...
        @Override protected <S extends Node> DescendantNodeStream<S> copyWithWalker(Filtermap<Node, ? extends S> filterMap, TreeWalker walker) {
        ...
        @Override protected <O> List<O> toListImpl(Filtermap<? super Node, ? extends O> filter) {
    ...
    static class DescendantStream extends FilteredDescendantStream<Node> {
        ...
        @Override public DescendantNodeStream<Node> crossFindBoundaries(boolean cross) {
    ...
    static class FilteredDescendantOrSelfStream<T extends Node> extends DescendantStreamBase<T> {
        ...
        @Override public Iterator<Node> baseIterator() {
    ...
    static class FilteredChildrenStream<T extends Node> extends AxisStream<T> {
        ...
        @Override public <R extends Node> NodeStream<R> flatMap(Function<? super T, ? extends @Nullable NodeStream<? extends R>> mapper) {
        ...
        @Override protected <O extends Node> @Nullable O lastImpl(Filtermap<? super Node, ? extends O> filter) {
    ...
    static class ChildrenStream extends FilteredChildrenStream<Node> {
        ...
        @Nullable @Override public Node last() {
            return len > 0 ? node.getChild(low + len - 1) : null;
        }

        @Nullable @Override public Node get(int n) {
            AssertionUtil.requireNonNegative("n", n);
            return len > 0 && n < len ? node.getChild(low + n) : null;
        }


        @Override public NodeStream<Node> take(int maxSize) {
            AssertionUtil.requireNonNegative("maxSize", maxSize);
            return StreamImpl.sliceChildren(node, filter, low, min(maxSize, len));
        }

        @Override public NodeStream<Node> drop(int n) {
            AssertionUtil.requireNonNegative("n", n);
            if (n == 0) {
                return this;
            }
        ...
        @Override public NodeStream<Node> dropLast(int n) {
            AssertionUtil.requireNonNegative("n", n);
            if (n == 0) {
                return this;
            }
