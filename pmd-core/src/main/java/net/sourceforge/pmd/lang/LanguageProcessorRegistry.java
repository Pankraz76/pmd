...
public final class LanguageProcessorRegistry implements AutoCloseable {
    ...
    private LanguageProcessorRegistry(Set<LanguageProcessor> processors) {
        this.processors = Collections
                .unmodifiableMap(CollectionUtil.associateBy(processors, LanguageProcessor::getLanguage));
     ...
     *             if the language is not part of this registry
     */
     ...
     *
     * @throws LanguageTerminationException
     *             If closing any of the processors threw something
     */
    @Override
     ...
     *             If the language depends on other languages, as they are then not
     *             included in this registry (see
    ...
    /**
     * Create a new instance by creating a processor for each language in the given
     * language registry. Each processor is created using the property bundle that
     * is in the map, if present. Language properties are defaulted to environment
     * variables if they are not already overridden.
     *
     * @throws IllegalStateException
     *             If any language in the registry depends on languages that are not
     *             found in it, or that could not be instantiated (see
     *             {@link Language#getDependencies()}).
     * @throws IllegalArgumentException
     *             If some entry in the map maps a language to an incompatible
     *             property bundle
     */
    public static LanguageProcessorRegistry create(LanguageRegistry registry,
            Map<Language, LanguagePropertyBundle> languageProperties, PmdReporter messageReporter) {
    ...
    // TODO this should be reused when implementing the CLI - see
    // https://github.com/pmd/pmd/issues/2947
    ...
    private static void setLanguageProperties(Map<Language, Properties> languageProperties, PmdReporter messageReporter,
            Language language, LanguagePropertyBundle properties) {
    ...
    private static <T> void trySetPropertyCapture(PropertySource source, PropertyDescriptor<T> propertyDescriptor,
            String propertyValue, PmdReporter reporter) {
        try {
            T value = propertyDescriptor.serializer().fromString(propertyValue);
            source.setProperty(propertyDescriptor, value);
        } catch (IllegalArgumentException e) {
            reporter.error("Cannot set property {0} to {1}: {2}", propertyDescriptor.name(), propertyValue,
                    e.getMessage());
        }
    }

    private static void readLanguagePropertiesFromEnv(LanguagePropertyBundle props, PmdReporter reporter,
            Map<String, String> env) {
        for (PropertyDescriptor<?> propertyDescriptor : props.getPropertyDescriptors()) {
            ...
            if (propertyValue != null) {
                if (props.isPropertyOverridden(propertyDescriptor)) {
                    ...
                    LOG.debug("Property {} for lang {} is already set, ignoring environment variable {}={}",
                            propertyDescriptor.name(), props.getLanguage().getId(), envVarName, propertyValue);
                } else {
                    LOG.debug("Property {} for lang {} is not yet set, using environment variable {}={}",
                            propertyDescriptor.name(), props.getLanguage().getId(), envVarName, propertyValue);
    ...
    /**
     * Returns the environment variable name that a user can set in order to
