...
final class SaxonExprTransformations {
    ...
    private static final SaxonExprVisitor ROOT_REDUCER = new SaxonExprVisitor() {
        ...
        public Expression visit(SlashExpression e) {
            ...
            if (right instanceof AxisExpression && ((AxisExpression) right).getAxis() == AxisInfo.CHILD
    ...
    /**
     * Turn {@code a/(b[c])} into {@code (a/b)[c]}. This is to somewhat normalize
     * the expression as Saxon parses this inconsistently.
     */
    ...
    /**
     * Turn {@code ((root)/descendant-or-self::node())/child::someTest} into
     * {@code ((root)/descendant::someTest)}. The latter is the pattern detected by
     * the rulechain analyser.
     */
     ...
     * 
     * This split will skip into any top-level lets. So, for "let $a := e1 in (e2 |
     * e3)" this will return the subexpression e2 and e3. To ensure the splits are
     * actually equivalent you will have to call
     * {@link #copyTopLevelLets(Expression, Expression)} on each subexpression to
     * turn them back into "let $a := e1 in e2" and "let $a := e1 in e3"
     * respectively.
     */
    ...
    /**
     * Wraps a given subexpression in all top-level lets from the original. If the
     * subexpression matches the original, then nothing is done.
     * 
     * @param subexpr
     *            The subexpression that has been manipulated.
     * @param original
     *            The original expression from which it was obtained by calling
     *            {@link #splitUnions(Expression)}.
     * @return The subexpression, wrapped in a copy of all top-level let expression
     *         from the original.
     */
    static Expression copyTopLevelLets(Expression subexpr, Expression original) {
        ...
        // Does it need them? Or is it already the same variable under the same
        // assignment?
