 ...
 * (for use mostly by {@link Renderer} instances). File IDs are used to identify
 * files e.g. in {@link RuleViolation}, {@link FileLocation}, {@link TextFile}.
 *
 * <p>
 * Note that the addressed file may not be an actual file on a file system. For
 * instance, you can create file ids from strings
 * ({@link #fromPathLikeString(String)}), or use {@link #STDIN} to address
 * standard input. The rendering methods of this interface (like
 * {@link #getAbsolutePath()}) do not have to return actual paths for those
 * exotic files, and operate on a best-effort basis.
 *
...
public interface FileId extends Comparable<FileId> {
    ...
    /**
     * The name used for an unknown file. This is mostly only relevant for unit
     * tests.
     */
    ...
    /**
     * Return the simple file name, like {@link Path#getFileName()}. This includes
     * the extension.
     */
    String getFileName();

    /**
     * Return the path as it was input by the user. This may be a relative or
     * absolute path.
     */
    String getOriginalPath();

    /**
     * Return an absolute path to this file in its containing file system. If the
     * file is in a zip file, then this returns a path from the zip root, and does
     * not include the path of the zip itself.
     */
    ...
    /**
     * If this file is in a nested filesystem (eg a zip file), return the file ID of
     * the container in the outer file system. Return null if this is in the root
     * file system.
     */
    @Nullable
    ...
    /**
     * This method is intentionally only meant for debugging, and its output is
     * unspecified. Code that needs a string representation should use one of the
     * named string conversion methods.
     */
    @Override
    String toString();

    /**
     * Return a path ID for the given string. The string is interpreted as a file
     * system path, so that {@link #getAbsolutePath()} and {@link #getUriString()}
     * may work.
     *
     * @param str
     *            A string. Should be a valid file system path for the platform (see
    ...
    static FileId fromPathLikeString(String str) {
        ...
        @Nullable
        Path fileNamePath = absPath.getFileName();
        return new FileId() {
     ...
     * {@link #fromPath(Path, FileId)} and defaults the second parameter to null.
     */
    ...
    /**
     * Return a file ID whose methods behave the same as the first parameter, and
     * whose {@link #getParentFsPath()} returns the second parameter.
     *
     * @param self
     *            A file id
     * @param parentFsPath
     ...
     * The path must be a valid path for this system
     * ({@link Paths#get(String, String...)} should not fail). The URI is rebuilt
     * using the outer file ID if it is non-null.
     *
     * @param absPath
    ...
    /**
     * Return a file ID for a URI. The URI must have scheme {@code file} or
     * {@code jar} and be a valid URI (see {@link URI#create(String)}). If the
     * scheme is {@code jar}, then the {@link #getParentFsPath()} is populated with
     * the path of the jar.
     *
     * @param uriStr
     *            A uri string
