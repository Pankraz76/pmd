...
/**
 * A contiguous range of text in a {@link TextDocument}. Empty regions may be
 * thought of like caret positions in an IDE. An empty region at offset
 * {@code n} does not contain the character at offset {@code n} in the document,
 * but if it were a caret, typing a character {@code c} would make {@code c} the
 * character at offset {@code n} in the document.
 *
 * <p>
 * Line and column information may be added by
 * {@link TextDocument#toLocation(TextRegion)}.
 *
 * <p>
 * Regions are not bound to a specific document, keeping a reference to them
 * does not prevent the document from being garbage-collected.
 *
 * <p>
 * Regions are represented as a simple offset+length tuple. In a document, valid
 * start offsets range from 0 to {@link TextDocument#getLength()} (inclusive).
 * The sum {@code startOffset + length} must range from {@code startOffset} to
 * {@link TextRegion#getLength()} (inclusive).
 *
 * <p>
 * Those rules make the region starting at {@link TextDocument#getLength()} with
 * length 0 a valid region (the caret position at the end of the document).
 *
 * <p>
 * For example, for a document of length 1 ({@code "c"}), there are only three
 * valid regions:
 * 
 * <pre>{@code
...
public final class TextRegion implements Comparable<TextRegion> {
    ...
    private static final Comparator<TextRegion> COMPARATOR = Comparator.comparingInt(TextRegion::getStartOffset)
     ...
     * between start offset and end offset. All characters have length 1, including
    ...
    /**
     * Returns true if the region contains no characters. In that case it can be
     * viewed as a caret position, and e.g. used for text insertion.
     */
    ...
    /**
     * Returns true if this region contains the character at the given offset. Note
     * that a region with length zero does not even contain the character at its
     * start offset.
     *
     * @param offset
    ...
    /**
     * Returns true if this region contains the entirety of the other region. Any
    ...
    /**
     * Returns true if this region overlaps the other region by at least one
     * character. This is a symmetric, reflexive relation.
     *
     * @param other
     *            Other region
     */
    ...
    /**
     * Returns a region that ends at the same point, but starts 'delta' characters
     * before this region. If the delta is negative, then this shifts the start of
     * the region to the right (but the end stays fixed).
     *
     * @throws AssertionError
     *             If the parameter cannot produce a valid region
     */
    ...
    /**
     * Returns a region that starts at the same point, but ends 'delta' characters
     * after this region. If the delta is negative, then this shifts the end of the
     * region to the left (but the start stays fixed).
     *
     * @throws AssertionError
     *             If the delta is negative and less than the length of this region
     */
    ...
    /**
     * Computes the intersection of this region with the other. This is the largest
     * region that this region and the parameter both contain. It may have length
     * zero, or not exist (if the regions are completely disjoint).
     *
    ...
    /**
     * Computes the union of this region with the other. This is the smallest region
     * that contains both this region and the parameter.
     *
     ...
     *
     * @throws AssertionError
     *             If either offset is negative, or the two offsets are not ordered
     */
     ...
     *
     * @param startOffset
     *            Offset for start and end of the position.
     *
     * @throws AssertionError
    ...
    /**
     * Checks that the parameters are a valid region, this is provided to debug,
     * will be a noop unless assertions are enabled.
     */
