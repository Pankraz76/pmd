...
/**
 * A handler for events occuring during analysis of a single file. Instances are
 * only used on a single thread for their entire lifetime, so don't need to be
 * synchronized to access state they own. File listeners are spawned by a
 * {@link GlobalAnalysisListener}.
 *
 * <p>
 * Listeners are assumed to be ready to receive events as soon as they are
 * constructed.
 *
...
public interface FileAnalysisListener extends AutoCloseable {
    ...
    /**
     * Signals the end of the analysis: no further calls will be made to this
     * listener. This is run in the thread the listener has been used in. This
     * means, if this routine merges some state into some global state of the
     * {@link GlobalAnalysisListener}, then that must be synchronized.
     *
     * @throws Exception
     *             If an exception occurs, e.g. IOException when writing to a
     *             renderer
     */
    @Override
