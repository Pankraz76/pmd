 ...
 * for each analyzed file, which themselves handle events like violations, in
 ...
 *
 * <p>
 * Listeners are the main API to obtain results of an analysis. The entry point
 * of the API ({@link PmdAnalysis}) runs a set of rules on a set of files. What
 * happens to events is entirely the concern of the listener.
 *
 * <p>
 * A useful kind of listener are the ones produced by
 * {@linkplain Renderer#newListener() renderers}. Another is the
 * {@linkplain GlobalReportBuilderListener report builder}.
 */
public interface GlobalAnalysisListener extends AutoCloseable {

    /**
     * Provides an initializer to gather analysis context before events start
     * occurring.
     * 
    ...
    /**
     * Returns a file listener that will handle events occurring during the analysis
     * of the given file. The new listener may receive events as soon as this method
     * returns. The analysis stops when the {@link FileAnalysisListener#close()}
     * method is called.
     *
     * @implSpec This routine may be called from several threads at once and needs
     *           to be thread-safe. But the returned listener will only be used in a
     *           single thread.
     *
     * @param file
     ...
     *             If {@link #close()} has already been called. This prevents
     *             manipulation mistakes but is not a strong requirement.
     */
    FileAnalysisListener startFileAnalysis(TextFile file);

    /**
     * Notify the implementation that the analysis ended, ie all files have been
     * processed. This listener won't be used after this is called.
     *
     * <p>
     * Closing listeners multiple times should have no effect.
     *
     * @throws Exception
     *             If an error occurs. For example, renderer listeners may throw
     *             {@link IOException}
     */
    @Override
    ...
    static GlobalAnalysisListener tee(Collection<? extends GlobalAnalysisListener> listeners) {
        ...
        // This prevents suppressed exceptions from being chained too deep if they occur
        // in close()
        List<GlobalAnalysisListener> myList = listeners.stream()
                ...
                .filter(l -> !(l instanceof NoopAnalysisListener)).collect(CollectionUtil.toUnmodifiableList());
    ...
    /**
     * A listener that just counts recorded violations. The result is available
     * after the listener is closed ({@link #getResult()}).
     */
    ...
    /**
     * A listener that throws processing errors when they occur. They are all thrown
     * as {@link FileAnalysisException}s. Config errors are ignored.
     *
     * <p>
     * This will abort the analysis on the first error, which is usually not what
     * you want to do, but is useful for unit tests.
     */
