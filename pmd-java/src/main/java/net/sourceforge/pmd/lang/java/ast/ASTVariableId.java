...
// @formatter:off
/**
 * Represents an identifier in the context of variable or parameter declarations
 * (not their use in expressions). Such a node declares a name in the scope it's
 * defined in, and can occur in the following contexts:
 *
 * <ul>
 ...
 *
 * <p>
 * Since this node conventionally represents the declared variable in PMD, it
 * owns a {@link JVariableSymbol} and can provide access to
...
public final class ASTVariableId extends AbstractTypedSymbolDeclarator<JVariableSymbol> implements ModifierOwner {
     ...
     * <ul>
     * <li>1. not necessarily the same as the type written out at the start of the
     * declaration, e.g. {@code int a[];}
     * <li>2. not necessarily the same as the types of other variables declared in
     * the same statement, e.g. {@code int a[], b;}.
     * </ul>
     *
     * <p>
     * These are consequences of Java's allowing programmers to declare additional
     * pairs of brackets on declarator ids. The type of the node returned by
     * {@link #getTypeNode()} doesn't take into account those additional array
     * dimensions, whereas this node's type takes into account the total number of
     * dimensions, i.e. those declared on this node plus those declared on the type
     * node.
     *
     * <p>
     * The returned type also takes into account whether this variable is a varargs
     * formal parameter.
     *
     * <p>
