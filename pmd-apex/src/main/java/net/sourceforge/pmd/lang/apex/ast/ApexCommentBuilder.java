...
final class ApexCommentBuilder {
    ...
    /**
     * Only remembers the node, to which the comment could belong. Since the
     * visiting order of the nodes does not match the source order, the nodes
     * appearing later in the source might be visiting first. The correct node will
     * then be visited afterwards, and since the distance to the comment is smaller,
     * it overrides the remembered node.
     *
     * @param node
     *            the potential parent node, to which the comment could belong
     */
    public void buildFormalComment(AbstractApexNode node) {
        if (!node.hasRealLoc()) {
            // Synthetic nodes such as "invoke" ASTMethod for trigger bodies don't have a
            // location in the
            // source code, since they are generated by the parser/compiler (see
            // ApexTreeBuilder)
            return;
        }
    ...
    private static class CommentInformation {
        ...
        CommentInformation(Map<Integer, String> suppressMap, List<Token> allCommentTokens) {
            ...
            this.docTokens = allCommentTokens.stream().filter(token -> token.getType() == ApexLexer.DOC_COMMENT)
                    .map(ApexDocToken::new).collect(toList());
            this.allCommentsByStartIndex = new TokenListByStartIndex(new ArrayList<>(allCommentTokens));
        }
    }

    /**
     * List that maps comment tokens to their start index without copy. This is used
     * to implement a "binary search by key" routine which unfortunately isn't in
     * the stdlib.
     *
     * <p>
