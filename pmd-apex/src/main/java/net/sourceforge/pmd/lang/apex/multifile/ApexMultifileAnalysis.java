...
/**
 * Stores multi-file analysis data. The 'Org' here is the primary ApexLink
 * structure for maintaining information about the Salesforce metadata. We load
 * 'Packages' into it to perform analysis. Once constructed you can get 'Issue'
 * information from it on what was found. The 'Org' holds mutable state for IDE
 * use that can get quite large (a few hundred MB on very large projects). An
 * alternative way to use this would be to cache the issues after packages are
 * loaded and throw away the 'Org'. That would be a better model if all you
 * wanted was the issues but more complex rules will need the ability to
 * traverse the internal graph of the 'Org'.
 *
 * <p>
 * Note: This is used by
 * {@link net.sourceforge.pmd.lang.apex.rule.design.UnusedMethodRule}.
 *
 * @author Kevin Jones
 */
public final class ApexMultifileAnalysis {
        ...
        try {
            ...
            if (rootDir.isPresent() && !rootDir.get().isEmpty()) {
                ...
                if (Files.isDirectory(projectPath) && Files.isRegularFile(sfdxProjectJson)) {
                    ...
                    // FIXME: Syntax & Semantic errors found during Org loading are not currently
                    // being reported. These
                    // should be routed to the new SemanticErrorReporter but that is not available
                    // for use just yet.
                    // Specifically we should check sfdx-project.json was ok as errors will disable
                    // further analysis
            ...
            // Note: Org.newOrg() will try to find the base Apex Types through the current
            // classloader
            // in package "com.nawforce.runforce". This requires, that directory listings
            // can be retrievied
            // on the URL that the classloader returns from
            // getResource("/com/nawforce/runforce"):
            ...
            // However, when running as an Eclipse plugin, we have a special bundle
            // classloader, that returns
            // URIs in the form "bundleresource://...". For the schema "bundleresource", no
            // FileSystemProvider can be
            // found, so we get a java.nio.file.ProviderNotFoundException. Since all this
            // happens during initialization of the class
            // com.nawforce.apexlink.types.platform.PlatformTypeDeclaration we get a
            // ExceptionInInitializerError
            // and later NoClassDefFoundErrors, because PlatformTypeDeclaration couldn't be
            // loaded.
    ...
    /**
     * Returns true if this is analysis index is in a failed state. This object is
     * then useless. The failed instance is returned from
     * {@link ApexLanguageProcessor#getMultiFileState()} if loading the org failed,
     * maybe because of malformed configuration.
     */
